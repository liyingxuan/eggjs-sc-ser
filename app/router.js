...
module.exports = app => {
  const { router } = app;
	 ...
	 *
	 * 【Example】
	 * 查询全部：  GET     http://localhost:7001/api/sc/
	 * 查询某条：  GET     http://localhost:7001/api/sc/[address]
	 * 新建数据：  POST    http://localhost:7001/api/sc/
	 * 更新数据：  PUT     http://localhost:7001/api/sc/[address]
	 * 删除数据：  DELETE  http://localhost:7001/api/sc/[address]
	 *
